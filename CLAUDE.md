...
isolated homes`, which MAT-378 broke by deleting the repo-level
`.claude-plugin/marketplace.json` the test installs from; in the fork,
`cli.spec.ts › attach <url> --extension`. Baseline first: stash your change,
rebuild if the artifact matters, re-run. Slow browser tests also contaminate
each other, so confirm a failure in isolation before treating it as real.
