...
This suite is green at 636/636 and the fork's extension suite has known
pre-existing failures (`cli.spec.ts › attach <url> --extension`). Baseline
first: stash your change, rebuild if the artifact matters, re-run. Slow browser
tests also contaminate each other, so confirm a failure in isolation before
treating it as real.
