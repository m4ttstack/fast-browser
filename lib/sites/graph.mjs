...
// `isReplayRecord` below) and (2) records whose `urlAfter` is missing or
// doesn't parse as an http(s) URL -- there is no usable destination to
// mine at all. Two other "no edge" outcomes are deliberately NOT counted
// here: `urlBefore === urlAfter` (nothing navigated) and a same-origin-
...
// rather than imported: `sweep.mjs` (Task 4) will import THIS module, so
