...
//
// **What counts as `skippedRecords` and what doesn't.** Four things
// increment it: (1) a record that isn't an object at all (hostile input,
// not a bug in the caller); (2) a replay record (see `isReplayRecord`,
// imported above); (3) a record whose `urlAfter` is missing or doesn't
// parse as an http(s) URL -- there is no usable destination to mine at
// all; and (4) a record with a real cross-URL transition on `origin` but a
// non-string `tool` -- `store.mjs`'s `parseAction` requires `tool` to be a
// string, so there is no valid action to record either. Two other "no
// edge" outcomes are deliberately NOT counted here: `urlBefore ===
// urlAfter` (nothing navigated) and a same-origin-unusable-but-parseable
// `urlAfter` that lands on a DIFFERENT origin (a cross-origin navigation
// away from `origin`). Both are ordinary, expected outcomes of feeding this
// miner a per-origin slice of a session that legitimately touched several
// origins (sweep.mjs's Task 4 groups records by their own origin before
// calling this per origin) -- counting them as "skipped" would make every
// multi-origin sweep look lossy for records that were never lost, just not
// this origin's edge.
//
