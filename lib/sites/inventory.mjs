...
// when `urlBefore` is usable AND already resolves to the origin being
// mined. `urlBefore` falls back to `urlAfter` ONLY when `urlBefore` itself
// is unusable (missing or unparseable) -- NEVER merely because it resolves
// to a different origin (fix round 1, Major F1): a record whose
// `urlBefore` is usable but CROSS-ORIGIN belongs to THAT origin's own
// group instead (`lib/flows/sweep.mjs`'s Task 4 grouping joins a record to
// every distinct canonical origin among its `urlBefore`/`urlAfter`, so that
// other group is guaranteed to exist) and contributes NOTHING here --
// falling back to `urlAfter` in that case would mis-file the interaction
// under the page it navigated TO rather than the page it actually happened
// ON. This mirrors graph.mjs's `from`/`to` split conceptually (urlBefore is
// "where this happened"), but inventory has no `to`: an interaction target
// belongs to the page it was found on, not wherever the action navigated
// next. Whichever URL is used for keying still has to resolve to `origin`
// itself (canonical `.origin` comparison, the binding carry-down from Task
// 1's review) -- a record that fails that check contributes nothing,
// exactly like graph.mjs drops a cross-origin `urlAfter`.
//
...
// contract, `isReplayRecord` imported from `trace-reader.mjs` (WS3b
// Task 1) -- previously a copy reproduced locally here and in graph.mjs/
// sweep.mjs, same anti-circularity reasoning graph.mjs documents for its
// own former copy, now consolidated to one shared home.
//
...
// Resolves the pattern a record's targets should be filed under: `urlBefore`
// when it's usable AND already on `origin`. Falls back to `urlAfter` ONLY
// when `urlBefore` itself is UNUSABLE (missing/unparseable) -- fix round 1,
// Major F1: a `urlBefore` that's usable but resolves to a DIFFERENT origin
// must NOT fall back to `urlAfter` (that would mis-file the interaction
// under the destination page instead of the page it actually happened on).
// Such a record instead resolves to the same unusable sentinel `urlInfo`
// itself returns for a truly unparseable URL -- the caller's existing
// `!keying.usable` check then correctly contributes nothing to THIS
// origin's inventory (the record belongs to its urlBefore's OWN origin
// group instead; sweep.mjs's Task 4 grouping guarantees that group exists).
function keyingUrlInfo(record, origin) {
  const before = urlInfo(record.urlBefore);
  if (before.usable) {
    return before.origin === origin ? before : { pattern: null, origin: null, usable: false };
  }
