...
async function manifestVersionFromSetting(setting, extensionId) {
  ...
  try {
    ...
    // so Chrome cannot be running it under this one.
...
async function settingFromPreferencesFile(profileDirectory, extensionId, filename) {
  try {
...
async function manifestVersionFromPreferencesFile(profileDirectory, extensionId, filename) {
  return manifestVersionFromSetting(
    await settingFromPreferencesFile(profileDirectory, extensionId, filename),
    extensionId,
  );
}
...
async function recordedFromWebStore(profileDirectory, extensionId) {
  for (const filename of ['Preferences', 'Secure Preferences']) {
    const setting = await settingFromPreferencesFile(profileDirectory, extensionId, filename);
    if (setting?.from_webstore === true) return true;
  }
  return false;
}
