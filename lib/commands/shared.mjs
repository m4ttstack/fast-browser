...
// includes ESC, 0x1b) and DEL (0x7f) from text that traces back to
// page-derived or otherwise schema-unvalidated content (a flow's
// `description`, a flow arg's own key name -- artifact.mjs's `parseArgs`
// accepts any string key) before it is ever written to a terminal: those
// bytes are how such text could repaint the screen, move the cursor, or
// fake other output rather than just display oddly. Tab (0x09) and
// newline (0x0a) are left alone. Display sanitization only -- never used
// on data that is compared, hashed, or persisted.
export function stripControlChars(value) {
  return typeof value === 'string' ? value.replace(/[\x00-\x08\x0b-\x1f\x7f]/g, '') : value;
}
