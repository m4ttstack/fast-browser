...
//
// `quirk add`/`list`/`remove` still have no confirmation prompt, unlike
// `flows approve` -- deliberate, not an oversight. The consent debt WS2b
// left open here is now discharged: WS3a's flow-runner clicks a quirk
// selector during replay ONLY as interrupt recovery, after a step's own
// full locator walk has already missed, proving the step's own action
// never fired. At most one attempt per step, click-only, and that click is
// never retried. See the WS3a Task 3 doc comment in
// `builtins/macros/flow-runner.js` (above `rawQuirks`) for the full
// consent basis.
//
...
// decides what counts as a usable url. Only `origin`/`pattern` are
// returned: callers that persist a `url` field (`digest`) build it as
// `origin + pattern`, never `origin + pathname`. `URL#pathname` never
// carries userinfo/query/fragment, but it CAN carry a token or id as a
// raw path segment (e.g. `/reset/<token>`), and that would reach disk
// verbatim if a raw pathname were ever used here -- `pattern`'s `:id`
// collapsing is what actually sanitizes it away (fix round 1, I1).
function canonicalizeUrl(value, stage) {
  ...
  return { origin: parsed.origin, pattern };
}
...
async function affordances(request, deps) {
  ...
  // A null urlPattern means the quirk was recorded origin-wide (`quirk add`
  // without --url-pattern), so it applies to every page on the origin,
  // including this one.
  const quirks = site.quirks.quirks.filter(
    (quirk) => quirk.urlPattern === pattern || quirk.urlPattern === null,
  );
  return {
    command: 'sites',
    sub: 'affordances',
...
async function digestSub(request, deps) {
  const { origin, pattern } = canonicalizeUrl(request.url, 'sites-digest');
  ...
  const record = {
    ...
    // and fragment are dropped, and the pathname itself is collapsed
    // through `pattern` (the same `:id`-collapsed shape used as the recall
    // key) rather than kept raw -- a raw pathname can still carry a
    // path-borne id/token (e.g. `/reset/<token>`) verbatim. Matches
