...
async function push(request, deps) {
  ...
  // outcome; everything at or after it was never attempted" boundary, which
  // concurrent in-flight requests could not give a truthful answer to. No
  // retry: one transport failure ends the whole push (a partial run is
  // always safe to re-issue in full -- see the failure message below,
  // identical flow bytes dedupe server-side, so already-ingested flows
  // from earlier batches are never double-counted by a re-push).
  const results = [];
  const notAttempted = [];
  let failure = null;
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex += 1) {
    ...
    if (failure) {
      ...
      // about them (created/rejected/clustered/deduped), never silently
      // dropped and never conflated with a real rejection.
  ...
  if (failure) {
    throw fail(
      ...
      `registry push failed on batch ${failure.batchIndex + 1} of ${batches.length}: ${failure.error.message} `
        + `${results.length} flow(s) were already pushed successfully in ${failure.batchIndex} earlier `
        + `batch(es); ${notAttempted.length} flow(s) were not attempted. Re-running `
      ...
      {
        partialState: {
          manifest, excluded, results, warnings, notAttempted, batches: batchMeta,
        },
      },
    );
  }
