...
import {
  flowFileName, flowId, parseFlow, serializeFlow,
} from '../flows/artifact.mjs';
import { lintArtifact } from '../../registry/lib/pii-lint.mjs';
import { stepSignature } from '../../registry/lib/signature-fields.mjs';
...
//
// This module IMPORTS three dependency-free registry/lib modules directly
// (verify, stepSignature, lintArtifact) -- registry/lib/signing.mjs and
// registry/lib/signature-fields.mjs and registry/lib/pii-lint.mjs are all
...
// dependencies. `ingest.mjs`'s alternates-union merge algorithm
// (mergeStep/unionLocators) is deliberately NOT imported -- this module
// mirrors that algorithm locally (see `mergeStep` below) rather than
// sharing code, since ingest.mjs merges INTO a canonical the client has no
// business depending on, while `pull` merges INTO the user's own local
// ready-tier file. Same algorithm, different owner.
//
...
// Security carry (task brief item 4): `registry/lib/signing.mjs`'s `verify`
// calls `createPublicKey` OUTSIDE its own try/catch -- garbage PEM input
// throws there, by that module's own design (a malformed key is a
// configuration bug, not a verification outcome). `pull` always feeds
// `verify` the PINNED key, which is safe only because THIS function
// validates it once, here, at the one place a peer-supplied key ever enters
// config: `registry init` receiving GET /health's `publicKey` field from a
// remote server. A garbage/malformed key at init time is rejected cleanly,
// before anything is ever pinned.
function assertValidPublicKeyPem(pem) {
  try {
    createPublicKey(pem);
  } catch {
    throw fail('registry-init', 'the registry returned a public key that could not be parsed.', 1);
  }
}
...
async function init(request, deps) {
  ...
  // Validate the PEER-supplied key before it is ever printed, compared, or
  // pinned -- see assertValidPublicKeyPem's own doc comment.
  assertValidPublicKeyPem(health.publicKey);

  const previousKey = current.registry.publicKey;
  const keyChanged = previousKey !== null && previousKey !== health.publicKey;
  const fingerprint = fingerprintPublicKey(health.publicKey);
  ...
  if (keyChanged) {
    ...
    deps.print(`Previous fingerprint: ${fingerprintPublicKey(previousKey)}`);
  ...
  const next = { ...current, registry: { url, publicKey: health.publicKey, assumeYes: false } };
...
async function push(request, deps) {
  ...
  if (!bypass) {
    // The MANIFEST: printed before the confirm prompt so the human sees
    // exactly what is about to leave the machine, same shape as flows.mjs's
    // `approve` printing flow details before its own typed-confirm gate.
    // Never printed in the --yes+assumeYes automation path -- that path is
    // designed for non-interactive/`--json` callers, and interleaving raw
    // text lines onto stdout ahead of a `--json` caller's final JSON report
    // would corrupt its machine-readable output; that path's callers get
    // the identical manifest data back through the report object instead.
    deps.print(`Registry push manifest (${manifest.length} flow(s)):`);
    if (manifest.length === 0) deps.print('(none)');
    for (const entry of manifest) deps.print(`${entry.name} - ${entry.origin}`);
    for (const entry of excluded) {
      for (const reason of entry.reasons) {
        deps.print(`Warning: ${entry.name} excluded - ${reason.path} (${reason.rule})`);
      }
    }
  ...
  if (included.length > 0) {
    ...
    const flows = included.map((flow) => ({
      artifact: flow,
      contentHash: sha256Hex(serializeFlow(flow)),
    }));
    const responseBody = await callRegistry(deps, { url: pushUrl, method: 'POST', body: { flows } });
    results = Array.isArray(responseBody?.results) ? responseBody.results : [];
  }

  return {
    command: 'registry', sub: 'push', manifest, excluded, results, warnings,
  };
}
...
async function pull(request, deps) {
  ...
  const existingNames = new Set(await deps.listFlowFiles(deps.paths.flowsDir));
  ...
  for (const envelope of envelopes) {
    ...
    if (!existingNames.has(fileName)) {
      await deps.writeFlowFile(path.join(deps.paths.flowsDir, fileName), serializeFlow(flow));
      existingNames.add(fileName);
      results.push({ name: flow.name, outcome: 'created' });
      continue;
    }

    let localFlow;
    try {
      localFlow = parseFlow(JSON.parse(await deps.readFlowFile(path.join(deps.paths.flowsDir, fileName))));
    } catch {
      warnings.push({ kind: 'pull-skipped', name: flow.name, reason: 'local ready flow could not be read' });
    ...
    // Same name, same stepSignature: union locator alternates into the
    // LOCAL flow (append-only), recompute its id from the merged content,
    ...
    await deps.writeFlowFile(path.join(deps.paths.flowsDir, fileName), serializeFlow(mergedFlow));
    results.push({ name: flow.name, outcome: 'merged' });
  }
