...
// Note for completeness, not a bug: neither this nor `defaultReadFlowFile`
// below calls `assertConfined`/`confineOrFail` the way `approve`/`reject`
// do before their mutating rename/delete, so a symlink planted inside a
// tier dir is followed and its target's content is shown by `find`/`list`
// exactly like a real file's would be. That is presentation only, never a
// promotion -- confinement guards the tier-boundary-crossing operations
// (approve moves pending -> ready, reject deletes), not read-only display.
// There is no escalation here either way: whoever has write access to a
// tier dir to plant a symlink already has write access to plant a real
// file with identical content, so a symlink buys them nothing they could
...
export async function flows(request, supplied = {}) {
  ...
  // Note only: an unknown subcommand here, and a bad flow name in
  // `assertValidFlowName` above, are semantically usage errors, not
  // lifecycle failures, but both still report as `LifecycleError` with
  // exit code 2 -- that is the existing CLI layering for every command in
  // this codebase, not something introduced here, so this stays aligned
  // with it rather than growing a separate usage-error type. Revisit only
  // if that layering itself changes.
