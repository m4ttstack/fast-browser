...
import {
  lstat, readFile, readdir, rename, unlink, writeFile,
...
// paths, and called a second time on the pending side immediately before
// the rename (the TOCTOU recheck, item 4), so a symlink swapped in during
// the human's think-time at the confirm prompt is caught too, not only one
// present at the initial read.
async function confineOrFail(deps, stage, rootDir, candidate, message) {
  try {
    await deps.assertConfined({ dataDir: deps.paths.dataDir, rootDir, candidate });
  } catch {
...
async function approve(request, deps) {
  ...
  // Fail-closed collision probe (item 3): ANY existing entry at readyPath --
  // file, directory, symlink, or one this process cannot read the content
  // of -- is a collision. The old design probed via `readFlowFile`, which
  // read a permission failure or a directory as "nothing there" and would
  // have let `moveFlow` rename over it.
  let readyExists;
  try {
  ...
  // TOCTOU recheck (item 4): re-confine and re-read the pending file right
  // before the mutating rename, and compare its id against what was shown
  // at the prompt. A human's think-time at the confirm prompt is an open
  // window; this closes it against both a content swap and the symlink-swap
  // variant of item 2 (confineOrFail refuses a symlink planted since the
  // first check).
  await confineOrFail(
    deps, 'flows-approve', deps.paths.flowsPendingDir, pendingPath,
    'the pending flow path is not safe to read.',
