...
async function performLockUpgrade({
  ...
  // A newer plugin can add a data-dir child; the upgrade is the only path an
  // existing install takes that is allowed to mutate, so it lands here.
...
export async function setup(request, supplied = {}) {
  ...
  try {
    ...
    try {
      ...
      //   registry.assumeYes  WS4b Task 7 review fix round 1, ruling: the
      //                     standing-consent bit for `registry push --yes`'s
      //                     bypass -- a reinstall is a re-consent point, so
      //                     this is forced back to false every time,
      //                     regardless of what the prior config held.
      //                     registry.url/registry.publicKey are NOT in this
      //                     list -- those stay carried (this branch has no
      //                     flag for them and no claim on the pinned
      //                     registry identity itself, only on whether an
      //                     unattended `--yes` push is still allowed to run
      ...
      config = parseConfig({
        ...
        registry: {
          ...(current?.registry ?? defaultConfig().registry),
          assumeYes: false,
        },
      });
      await deps.saveConfig(deps.paths, config);
