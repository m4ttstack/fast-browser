...
import { DOCTOR_CHECK_IDS, DOCTOR_CHECK_PREREQUISITES, defaultCheck } from '../doctor/checks.mjs';
...
import { RETIRED_EXTENSION_IDS, detectChromeExtension } from '../extension/detect.mjs';
...
async function productionDependencies(request, dependencies, paths) {
  ...
  const checks = {
    ...
    'extension-installed': async () => {
      ...
      let mismatchedStoreVersion = null;
      for (const profile of profiles) {
        if (profile.installed && profile.fromWebStore && profile.manifestVersion !== lock.extension.version) {
          mismatchedStoreVersion ??= profile.manifestVersion;
        }
        if (!profile.installed || profile.manifestVersion !== lock.extension.version) continue;
        versionMatchedSomeProfile = true;
        if (profile.fromWebStore) {
          // Chrome rewrites a store install, so it never byte-matches the
          // unpacked artifact; the pinned id and version identify it.
          return pass('The pinned Chrome extension is installed from the Chrome Web Store.');
        }
        if (await verify({ loadedPath: profile.path, managedDirectory })) {
          return pass('The pinned Chrome extension is installed.');
        }
      }
      if (versionMatchedSomeProfile) {
        return fail(
      ...
      // clears, and loading the unpacked build instead would give up
      // auto-update. Chrome never downgrades, so a copy ahead of the lock
      // means Fast Browser itself is the one behind.
      if (mismatchedStoreVersion !== null) {
        const pinned = lock.extension.version;
        return mismatchedStoreVersion.localeCompare(pinned, undefined, { numeric: true }) > 0
          ? fail(
            `The Chrome Web Store copy (${mismatchedStoreVersion}) is newer than the ${pinned} this Fast Browser pins.`,
            'Update Fast Browser, then run doctor again.',
          )
          : fail(
            `The Chrome Web Store copy is at ${mismatchedStoreVersion}; this Fast Browser pins ${pinned}.`,
            `Chrome updates it once the store has ${pinned}. To check now, open chrome://extensions, turn on Developer mode, and click Update.`,
          );
      }
      return fail(
        'The pinned Chrome extension is not installed.',
        `Open chrome://extensions, turn on Developer mode, click Load unpacked, and choose ${managedDirectory}.`,
      );
    },
    ...
    // Separate from extension-installed for the same reason extension-loaded
    // is: it answers a question about a DIFFERENT extension. A profile can
    // hold the retired id and the current one at once, and Chrome then runs
    // two Fast Browser extensions that both claim the toolbar and the relay.
    // Setup cannot fix it -- removing an extension is Chrome's own list, not
    // a file on disk -- so this fails only to name the id to remove.
    'retired-extension': async () => {
      const detect = dependencies.detectChromeExtension ?? detectChromeExtension;
      const stale = [];
      for (const extensionId of RETIRED_EXTENSION_IDS) {
        const profiles = await detect({ extensionId, chromeUserDataDir });
        if (profiles.some((profile) => profile.installed)) stale.push(extensionId);
      }
      return stale.length === 0
        ? pass('No retired Fast Browser extension is installed.')
        : fail(
          `A retired Fast Browser extension is still installed: ${stale.join(', ')}.`,
          'Open chrome://extensions, remove the extension with that ID, and load'
          + ' the current one from the managed install directory.',
        );
    },
    pairing: async () => (
...
export async function doctor(request = {}, dependencies = {}) {
  ...
  const context = {};
  const checks = [];
  const selected = new Set((request.checks ?? DOCTOR_CHECK_IDS).flatMap((id) => [
    id,
    ...(DOCTOR_CHECK_PREREQUISITES[id] ?? []),
  ]));
  for (const id of DOCTOR_CHECK_IDS.filter((checkId) => selected.has(checkId))) {
