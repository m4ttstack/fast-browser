...
async function productionDependencies(request, dependencies, paths) {
  ...
  const checks = {
    ...
    'extension-installed': async () => {
      ...
      // clears; loading the unpacked build instead would give up auto-update.
      if (trailingStoreVersion !== null) {
        return fail(
          `The Chrome Web Store copy is at ${trailingStoreVersion}; this Fast Browser pins ${lock.extension.version}.`,
          `Chrome updates it once the store has ${lock.extension.version}. To check now, open chrome://extensions, turn on Developer mode, and click Update.`,
        );
      }
      return fail(
