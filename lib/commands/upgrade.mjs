...
// retired-extension is the same shape of thing: an extension Chrome still
// holds under an id this project can no longer sign for. Only the user can
// remove it, from Chrome's own list, so setup can neither cause it nor fix it
// and it is never evidence that the pinned artifacts drifted.
export const MANUAL_STEP_CHECK_IDS = Object.freeze(new Set(['extension-loaded', 'retired-extension']));
...
// Checks whose verdict comes from running a real agent (browser-driver
// shells out to a live `codex exec` smoke run with a 60s budget). A live
// model call can fail for reasons that say nothing about this machine's
// managed state: a timeout, a transient provider error, a malformed event
// stream. Counting such a failure as drift vetoed a fully-explained runtime
// upgrade at the classification gate (live, 2026-08-05: four artifact checks
// correctly explained by the alpha.9 -> alpha.10 pin bump, refused because
// the smoke run happened to flake in the same report). The check is also
// orthogonal to the pinned artifacts by construction -- its prompt forbids
// browser tools, and its remediation is `fast-browser configure`, which owns
// the agent file setup never touches. So: never drift evidence, never
// upgrade-explainable, exactly the DRIFT_EXEMPT contract.
export const AGENT_SMOKE_CHECK_IDS = Object.freeze(new Set(['browser-driver']));
