...
//   reasons }] sorted best score first, ties broken by flow name ascending,
//   capped at MAX_RESULTS. `flows` is [{ flow, tier }] (Task 3's parsed
...
export function matchFlows({
  ...
  for (const entry of flows) {
    ...
    let score = scoreFlow(flow, { url, intentTokens });
    const reasons = [];
