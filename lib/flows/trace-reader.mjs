...
// "Directory layout"). Anything else under the output dir -- other files, other
...
//
// Precondition: `byteOffset` must be `0` or a value this function itself
// previously returned as `endByte` for this same session -- it always lands
// just past a complete line's `\n`, on the start of the next line (or EOF).
// A `byteOffset` that instead lands mid-line (a caller-side cursor bug) is
// NOT validated or rejected: the mis-started fragment simply fails
// `JSON.parse` like any other malformed line, gets counted toward
// `skipped`/`lineCount`, and the scan resyncs cleanly at the next real `\n`
// (JSON lines never contain a raw, unescaped 0x0A byte). Safe, but silent --
// a cursor bug upstream would degrade into quietly dropped records rather
// than an error. `startedAtLineBoundary` (WS3b Task 2 fold-in) gives a
// caller a cheap, genuine way to catch exactly this: `true` when
// `byteOffset` is `0` or the byte immediately before it is `\n` (0x0A) --
// i.e. `byteOffset` actually sits on a line start in the CURRENT file --
// `false` otherwise. It is computed directly off the buffer this call
// already read, so it costs nothing extra. It catches a stale cursor that
// no longer lands on a line boundary at all (the file was truncated
// mid-line and rewritten with different, shorter content, or `byteOffset`
// was never a real endByte to begin with) -- landing exactly mid-line, as
// the paragraph above describes. It does NOT catch every rewrite: a
// same-length (or coincidentally boundary-preserving) rewrite that
// replaces one complete line with different complete-line content still
// starts at a `\n` and reports `true` -- content-level verification is out
// of scope for a check this cheap, and a caller that needs it must fall
// back to a full read for other reasons (a stat-based file-size check, for
// instance -- see `lib/flows/sweep.mjs`'s `tryByteCursorRead`).
export async function readTraceRecordsFrom(sessionDir, byteOffset) {
  ...
  try {
    ...
    // No buffer exists to check a nonzero `byteOffset` against in either
    // branch below -- `startedAtLineBoundary` degrades to the only thing
    // that's still verifiable without one: whether `byteOffset` is the
    // trivially-always-a-boundary `0`.
    if (error?.code === 'ENOENT') {
      return {
        ...
        endByte: byteOffset,
        lineCount: 0,
        startedAtLineBoundary: byteOffset === 0,
      };
    }
    return {
      records: [],
  ...
  // `byteOffset - 1` reads the byte just before the requested start --
  // `undefined` (not a throw) when `byteOffset` is beyond `buffer.length`,
  // which correctly resolves to `false` below (an out-of-bounds start is
  // never "on a line boundary" in any useful sense).
  const startedAtLineBoundary = byteOffset === 0 || buffer[byteOffset - 1] === 0x0a;
  return {
    records, skipped, readable: true, endByte, lineCount, startedAtLineBoundary,
  };
}
...
// Scans the runtime's output dir for trace-<epochMs> children -- the shape TraceLog.create
...
// perfectly readable.
export async function listTraceSessions(outputDir) {
  let entries;
  try {
