...
export async function readTraceRecords(sessionDir) {
  ...
  try {
    ...
    // `readable: false` distinguishes THIS -- couldn't read the file at
    // all, for any reason (ENOENT, EACCES, any other fs error) -- from a
    // file that was read successfully and simply turned out to be empty
    // or all-garbage (readable: true, records: []). A caller doing
    // incremental bookkeeping off this result (lib/flows/sweep.mjs) needs
    // that distinction: a transient/permissions read fault must never be
    // treated as "nothing happened in this session yet," since sweep.mjs
    // would otherwise rewind that session's cursor to zero and re-count
    // provenance it had already applied once the file becomes readable
    // again.
    return { records: [], skipped: 0, readable: false };
  }
  ...
  return { records, skipped, readable: true };
}
