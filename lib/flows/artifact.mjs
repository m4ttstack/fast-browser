...
// MAT-149: must match flow-runner.js's own `{arg}` substitution regex
// (`TOKEN`, `/\{([A-Za-z_][A-Za-z0-9_]*)\}/g`) exactly -- an arg key this
// module accepts but that regex can never match would replay with its
// `{name}` placeholder never substituted. The compiler (compile.mjs)
// sanitizes every minted name to this same shape at mint time, so this is
// the other end of that guarantee: an artifact that somehow carries a key
// outside it (a stale compiler, a hand-edited/hand-authored file) fails to
// parse loudly instead of silently compiling into a flow that can never
// replay correctly.
...
function parseStep(value, index) {
  ...
  switch (op) {
    case 'goto': {
      // No waitAfter slot: the contract only defines op/url for goto (see
      // rejectUnknownKeys below), so a goto step can never carry a
      // step-level waitAfter. This is contract-forced, not an oversight --
      // the runner already waits for domcontentloaded after every goto, so
      // there is nothing for a waitAfter to add here.
    ...
    case 'press': {
      const step = { op };
      // No current capture path populates a target for this op --
      // TRACE.md's browser_press_key sends a keyboard event, not an
      // enriched target -- so a compiled press step never has one.
      // `target` stays optional for hand-authored or future flows that
      // pair a press with a resolved locator.
    ...
    case 'upload': {
      const step = { op };
      // TRACE.md only enriches targets for a fixed set of tools that does
      // not include browser_file_upload, so a compiled upload step never
      // has a resolved locator -- the MCP upload model is file-chooser
      // based (a preceding click opens the chooser), so replay needs no
      // locator either. `target` stays optional for hand-authored flows.
      if (Object.hasOwn(record, 'target')) step.target = parseTarget(record.target, `${field}.target`);
...
function parseArgs(value, field) {
  ...
  const args = {};
  for (const key of Object.keys(record)) {
    // Own-property-safe: JSON.parse happily produces an own key literally
    // named "__proto__" (it uses CreateDataProperty, not assignment), but
    // `args[key] = ...` below for that key would hit the inherited
    // accessor on Object.prototype and reassign this object's prototype
    // instead of adding an own property -- the arg would silently vanish
    // from the returned flow with no error, the opposite of this module's
    // fail-loudly contract. Reject it by name instead, matching
    // resolvePalette's __proto__ guard in lib/annotate/palette.mjs.
    if (key === '__proto__') {
      throw new FlowError(`${field}.${key} is not a valid argument name`);
    }
    const entryField = `${field}.${key}`;
    if (!ARG_NAME_PATTERN.test(key)) {
      throw new FlowError(`${entryField} is not a valid argument name`);
    }
...
// built.
// Note only, no behavior change: a literal `__proto__` key survives
// `Object.keys`/the sort above, but `sorted[key] = ...` for that exact key
// name is bracket-notation assignment, which JS special-cases to set the
// object's prototype rather than create an own property -- assigning a
// non-object/non-null value (a plain string) to it is a silent no-op, so
// the key vanishes from `sorted` instead of round-tripping. This is
// harmless in practice: the one place a step value could plausibly carry a
// literal `__proto__` key is a js step's `script.args`, and by the time
// canonicalize ever sees it, `redactScriptArgs` (compile.mjs) has already
// replaced every value there with a string placeholder -- there is no real
// secret behind the key this could drop.
