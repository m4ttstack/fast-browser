...
async function tryByteCursorRead(sessionDir, previous) {
  ...
  // must be consistent with what this offset read reports). Every valid
  // record this read found is exactly one raw line, and `lineCount` counts
  // EVERY raw line consumed -- valid, blank, or malformed alike -- so
  // `records.length` can never exceed `lineCount` for a genuine
  // continuation of this same file from this same byte offset. A
  // violation is mathematically impossible unless the cursor no longer
  // describes this file's actual history (e.g. truncated then overwritten
  // with different, still well-formed content at the same or a larger
  // size -- which the file-size check above cannot catch on its own):
  // treated exactly like any other insane cursor, falling back rather than
  // trusting a self-contradictory read. This is necessarily a narrower net
  // than a full content re-verification (which would defeat the point of
  // the optimization); the residual gap is the same class of accepted,
  // unmodeled edge case as the module's other restore/rewrite notes above
  // (F8, the shrunk-session guard) -- actions.jsonl is append-only in every
  // real capture path.
  if (offsetResult.records.length > offsetResult.lineCount) return null;
...
export async function sweep({ paths, now } = {}) {
  ...
  for (const session of sessions) {
    ...
    // `endByte` is only trustworthy as "the byte offset for `observedTotal`
    // records" -- when `nextLines` didn't actually reach `observedTotal`
    // (a still-live session, compile deferred; or a shrink the guard just
    // absorbed), the compile-side byte cursor stays exactly where it was
    // too, same as `nextLines` staying at `previousLines`. An old-format
    // entry with no prior `bytes` at all, and no progress to report this
    // sweep either, has nothing correct to fall back to -- `0` is always
    // safe there (`byteCursorsSane`'s "never used to skip content" note):
    // it just costs one more full read next sweep, never lost data.
    const nextBytes = nextLines === observedTotal
      ? endByte
      : (Number.isInteger(previous?.bytes) ? previous.bytes : 0);
    ...
    // `previousLines` (always <= `previousProvenanceLines`), so the
    // provenance-only tail is a further slice into it.
    const provenanceSlice = delta.slice(previousProvenanceLines - previousLines);
    ...
    // provenance cursor, and the same byte-mirroring as `nextBytes` above.
    const nextProvenanceLines = Math.max(observedTotal, previousProvenanceLines);
    const nextProvenanceBytes = nextProvenanceLines === observedTotal
      ? endByte
      : (Number.isInteger(previous?.provenanceBytes) ? previous.provenanceBytes : 0);
    ...
    const nextEntry = {
      ...
      bytes: nextBytes,
      provenanceBytes: nextProvenanceBytes,
    };
