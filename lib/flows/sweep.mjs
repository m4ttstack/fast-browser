...
import {
  appendFile, chmod, readdir, readFile, rename, stat, unlink, writeFile,
...
import { flowsAreCoJourney, journeyKey, mergeCoJourney } from './cluster.mjs';
import { compileSession } from './compile.mjs';
import { applyHeal, parseFailurePayload, proposeHeal } from './heal.mjs';
import { isReplayRecord, listTraceSessions, readTraceRecordsFrom } from './trace-reader.mjs';
import {
...
// land on the same value. Both cursors count entries in `trace-reader.mjs`'s
// RETURNED `records` array (schema-valid records only), not raw JSONL line
...
// records stay an exact, stable PREFIX of any later re-parse. WS3b Task 1
// added `readTraceRecordsFrom(sessionDir, byteOffset)`, a genuine
// partial/offset read; WS3b Task 2 (below) threads a BYTE-offset cache
// alongside these two record-count cursors so a sweep can resume there
// instead of re-parsing from zero every time -- the two-cursor LINE model
// described above is unchanged and stays authoritative either way.
//
...
//
// --- byte cursors: a CACHE over the same two-cursor LINE model (WS3b
// Task 2) ---
//
// `bytes`/`provenanceBytes` mirror `lines`/`provenanceLines` one-for-one,
// in byte-offset space instead of valid-record-count space, purely so a
// sweep that already knows how far it got last time can resume
// `readTraceRecordsFrom` at that BYTE instead of re-scanning/re-parsing the
// whole file from zero every sweep. They are NEVER authoritative and never
// used to skip content on their own: `lines`/`provenanceLines` stay the
// single source of truth for "how much of this session has been consumed"
// (Global Constraints). See `byteCursorsSane`/`tryByteCursorRead`/
// `readSessionSinceLines` below for the read path, and the session loop for
// how `nextBytes`/`nextProvenanceBytes` are derived -- they only ever
// advance in lockstep with `nextLines`/`nextProvenanceLines`, using the
// exact same shrink-guard philosophy the F8/Task-8 notes above already
// established for the line cursors. A byte cursor that is absent (old
// pre-Task-2 state), malformed, out of order (`bytes > provenanceBytes`,
// which should never happen by construction but a hand-edited/corrupted
// state file could produce), or no longer fits the file's actual current
// size (truncated/replaced) is simply not trusted: the session falls back
// to a full read from byte zero -- exactly what every sweep already did
// before this fix -- and backfills a fresh, correct byte cursor from that
// read's own result. Lost/skipped content from a bad byte cursor is not an
// accepted risk here; it is a correctness bug this fallback exists
...
//
// `readTraceRecordsFrom` distinguishes a session it genuinely found empty
...
//
// `readTraceRecordsFrom` carves ENOENT (no actions.jsonl at all) out of the
...
// very next sweep (a vanished file has no valid byte cursor to resume from
// either -- WS3b Task 2's `tryByteCursorRead` falls back to the same full
// read this always used, for the same reason). This session does NOT take
// the `!readable` early-continue path above (it IS readable), so without a
// separate guard the plain `nextLines = records.length` /
// `nextProvenanceLines = records.length` assignments would happily write a
// SMALLER cursor back to state -- rewinding it -- and a later sweep that
// finds the file restored (or re-grown) would re-slice and re-count
// everything between the rewound cursor and the restored content,
// including replays already applied once. The fix: both cursor assignments
// are `Math.max(observedTotal, previous*Lines)` (WS3b Task 2 renamed the
// right-hand side from a literal `records.length` to `observedTotal` --
// the same quantity, now sourced from whichever read path ran -- when the
// byte-cursor path is in play; the guard itself is unchanged) -- never
// write a cursor smaller than what's already saved. Concretely this means
// a shrunk session is treated as "no new lines this sweep" (its
// compile/provenance slices are already `[]`, since `records.length >
// previousLines` is false) with the cursor left
...
// F8 never touches.
//
// --- site memory mining (WS2b plan, Task 4; single-pass restored WS3b
// Task 2) ---
//
// Once a session compiles (the branch above, `isComplete` with at least one
// non-empty compile chunk), the compile chunks -- CONCATENATED back
// together, replays already excluded by `splitAtReplayBoundaries` -- are
// also handed to the graph (`lib/sites/graph.mjs`) and inventory
// (`lib/sites/inventory.mjs`) miners, in exactly ONE `mineSiteMemory` call
// per session. This is deliberately built from the compile branch's own
// slice, not a new cursor: site memory inherits the two-cursor model and
// live-session deferral for free by piggybacking on `lines`, rather than
// re-deriving either. A LIVE session therefore mines nothing (compilation,
// and mining with it, is deferred until `meta.endedAt` exists -- see
// above), and a slice that is ALL replay records mines nothing either (no
// compile chunks are produced from an all-replay slice to begin with; the
// miners' own replay self-exclusion is harmless overlap on top of that,
// never load-bearing here).
//
// WS3a (MAT-137, "replay records as segment boundaries") split the compile
// slice into per-replay CHUNKS so a replay could close a compile segment
// without stitching unrelated actions into one flow (`splitAtReplayBoundaries`
// below), and mining was wired to run once PER CHUNK alongside each
// chunk's own `compileSession` call. That over-applied the split to mining,
// which has no segment-boundary concept of its own: a target or edge
// appearing on BOTH sides of a mid-session replay got mined (and merged
// into the site store) once per chunk instead of once for the session,
// double-counting it in the sweep's own `sites` report and reading/writing
// the store once per chunk instead of once per session. Segment boundaries
// stay compile-only (immediately below, unchanged); mining concatenates
// every chunk's records back into one flat list first, restoring the exact
// single-pass counts a pre-WS3a (WS2b) sweep would have reported for the
// same session.
//
...
// --- runs ledger + heal-degrade warnings (WS4a drift-harness plan, Task 7) ---
//
// Two additive, best-effort surfaces layered on top of the replay-provenance
// pass above -- neither changes cursor/provenance/heal semantics, and
// neither can fail the sweep itself.
//
// **Runs ledger**: one JSONL line appended to `paths.runsFile` per
// replay record this sweep resolves to a known artifact (a record
// `resolveReplayTarget` cannot match still counts toward `replaysSeen`
// above, but has no flow identity to report against, so it gets no ledger
// line -- same "nothing to say" posture as the ordinary provenance path
// silently skipping it). Record shape (plan Scope decisions, exact):
// `{ v: 1, ts, flowName, flowId, origin, outcome, quirkAttempted,
// healedKind, sessionDir }`.
//
// `outcome` precedence for THIS record: `'healed'` when this record's own
// failure produced an accepted (durably written) heal; `'failed'` for any
// other failure; `'clean'` for a success. **`'fallback'`/`'escalated'`/
// `'quirk-recovered'` are real rungs on the healing ladder (see the
// drift-harness's own `classifyReplay`, tests/e2e/helpers/drift-harness.mjs)
// but are NOT reachable from this module** -- they are honest, deliberate
// omissions, not an oversight: `lib/flows/trace-reader.mjs`'s `TraceRecord`
// (packages/fast-browser-mcp/TRACE.md) carries no field capturing a
// successful `browser_run_code_unsafe` call's own RETURN value (only
// `error`, set on a thrown/rejected call) -- the macro's own `{ ok,
// locatorFallbacks, ... }` shape (builtins/macros/flow-runner.js) is never
// persisted to `actions.jsonl` at all. A sweep reading only trace records
// therefore cannot distinguish a clean success from one that needed a
// locator fallback, an escalated probe, or a quirk dismissal -- every
// success looks identical from here: `record.error` absent. Per this
// task's own Global Constraints ("no macro changes", and WS4a plans no
// runtime-fork release), that capture gap is out of scope to close here;
// widening it is a future runtime-capture task, not a ledger-shape change
// (the record shape already reserves the richer outcome values for when
// that lands). `quirkAttempted` is populated whenever `parseFailurePayload`
// recovers a payload carrying flow-runner.js's own `quirkAttempted` key
// (already emitted inside every FAILURE shape today -- no new payload key)
// -- so, like the outcome values above, it can only ever be non-null on a
// `'failed'`/`'healed'` record, never on a `'clean'` one (a success has no
// failure payload to parse at all).
//
// A ledger write failure -- `ensurePrivateDirectory`/`appendFile` throwing
// -- is caught and reported in `report.runsErrors` (mirrors `healErrors`'s
// own containment exactly, module doc comment's "healing" section): the
// sweep continues untouched, the cursor still advances, nothing about
// provenance or healing is affected. The ledger is derived data (plan
// Scope decisions): losing an append loses `stats` visibility for that one
// replay, never correctness of the flow artifacts themselves.
async function appendRunRecord(paths, entry, runsErrors, sessionDir) {
  try {
    await ensurePrivateDirectory(paths.runsDir);
    await appendFile(paths.runsFile, `${JSON.stringify(entry)}\n`, { mode: 0o600 });
  } catch (error) {
    runsErrors.push({ sessionDir, error: error?.message ?? String(error) });
  }
}

// **Heal-degrade warnings**: `heal.mjs`'s own `runRanker` already degrades
// ANY injected-ranker failure to the lexical default silently (its own doc
// comment: "ANY encoder failure silently degrades that call to lexical") --
// that degrade is the right behavior for `proposeHeal` to keep, but it left
// no visible trace for a caller until now. Rather than change `heal.mjs`'s
// return contract (its `decision | null` shape is exercised by many pinned
// tests, and `runRanker`/`finalize`'s internal `usedFallback` never escapes
// `proposeHeal` today), this wraps the RANKER FUNCTION ITSELF before it
// ever reaches `proposeHeal`: the wrapper calls through to the real ranker
// unchanged and simply observes whether it throws/rejects, pushing one
// `{ kind: 'encoder-degraded', reason }` entry (same shape `find`'s own
// rerank degrade already uses, lib/commands/flows.mjs's `rerankByBand`)
// before RE-THROWING the exact same error -- so `heal.mjs`'s own
// `runRanker` still does the actual lexical-fallback ranking immediately
// afterward, byte-for-byte as it always has; this wrapper only observes,
// never changes the outcome. `minScore`/`minMargin` (fix round 1's
// ranker-supplied-thresholds mechanism) are copied onto the wrapper so
// `proposeHeal`'s `finalize` reads the exact same thresholds it would have
// off the unwrapped ranker. A `ranker` that is `undefined` (no encoder
// configured -- the ordinary lexical-only path) passes through unchanged:
// there is no "fallback" to observe when nothing but lexical was ever
// asked for.
function warningTrackingRanker(ranker, warnings) {
  if (typeof ranker !== 'function') return ranker;
  const reportDegrade = (error) => {
  ...
  if (Number.isFinite(ranker.minScore)) tracked.minScore = ranker.minScore;
  if (Number.isFinite(ranker.minMargin)) tracked.minMargin = ranker.minMargin;
  return tracked;
}

// flow-runner.js's second failure contract: a lost cdp connection, as opposed
// to a step that did not work. Kept as a literal here rather than imported
// because the macro is a sandboxed source file with no module surface to
// import from, the same reason heal.mjs keeps its own FLOW_RUNNER_FAILURE
// literal.
const SIDECAR_LOST_PREFIX = 'SIDECAR_LOST: ';
...
function validProcessedEntry(value) {
  ...
  // WS3b Task 2: `bytes`/`provenanceBytes` are a cache alongside the
  // authoritative `lines`/`provenanceLines` above (Shared shapes) -- kept
  // only when individually well-shaped (non-negative integer). Unlike
  // `provenanceLines` above, a missing/malformed byte cursor is dropped
  // rather than defaulted: there is no safe value to invent the way
  // "provenanceLines defaults to lines" is safe for the old one-cursor
  // shape. A dropped/absent byte cursor makes the session loop's
  // `byteCursorsSane` check fail, which falls back to a full read this
  // sweep and backfills a fresh, correct value from it.
  if (Number.isInteger(value.bytes) && value.bytes >= 0) entry.bytes = value.bytes;
  if (Number.isInteger(value.provenanceBytes) && value.provenanceBytes >= 0) {
    entry.provenanceBytes = value.provenanceBytes;
  }
  return entry;
}
...
// --- WS3b Task 2: byte-cursor resume ---

// Shape/order sanity only -- see the module doc comment's "byte cursors"
// section. Whether the resulting offset is actually safe to READ from
// (fits the file's current size) is checked separately in
// `tryByteCursorRead`, which needs an `await stat(...)` this synchronous
// check deliberately stays free of.
function byteCursorsSane(previous) {
  return Number.isInteger(previous?.bytes) && previous.bytes >= 0
    && Number.isInteger(previous?.provenanceBytes) && previous.provenanceBytes >= 0
    && previous.bytes <= previous.provenanceBytes;
}

// Attempts the optimized read: resumes `readTraceRecordsFrom` at
// `min(bytes, provenanceBytes)` (Shared shapes) -- which, by the same
// `provenanceLines >= lines` construction the byte cursors mirror
// (`provenanceBytes >= bytes` always in practice), is always just
// `previous.bytes`; the literal `Math.min` is kept anyway as the honest
// expression of the documented rule rather than silently assuming the
// invariant always holds. Returns `null` when the cursor isn't safe to
// trust for this call (never throws) -- the caller falls back to a full
// read from byte zero. Returns `{ readable: false }` for a genuine fs read
// fault: the caller must NOT retry with a fallback full read (module doc's
// "unreadable sessions" note -- `readTraceRecordsFrom` always reads the
// WHOLE file into a buffer regardless of offset, so a second attempt would
// hit the exact same fault; the session is left completely untouched
// either way).
async function tryByteCursorRead(sessionDir, previous) {
  const minOffset = Math.min(previous.bytes, previous.provenanceBytes);
  let fileSize;
  try {
    ({ size: fileSize } = await stat(path.join(sessionDir, 'actions.jsonl')));
  } catch {
    // ENOENT (file vanished) or any other stat fault: nothing to safely
    // check this cursor against. Falls through to the full-read fallback,
    // which already has its own honest handling for both cases (ENOENT
    // reads as an empty session via trace-reader's own carve-out; any
    // other fault reports unreadable).
    return null;
  }
  // The larger cursor must still fit inside the file as it actually is
  // right now. A byte cursor pointing past EOF means the file is smaller
  // than whatever this cursor was computed against -- truncated, replaced,
  // or otherwise no longer the file this cursor describes. Trusting it
  // anyway would silently read nothing (`Buffer#indexOf` past the buffer's
  // length just returns -1 -- trace-reader.mjs), which is exactly the
  // "corrupted cursor loses data" failure mode this check exists to rule
  // out.
  if (previous.provenanceBytes > fileSize) return null;

  const offsetResult = await readTraceRecordsFrom(sessionDir, minOffset);
  if (!offsetResult.readable) return { readable: false };

  // Structural sanity (Shared shapes' sanity rule: the records consumed
  // must be consistent with what this offset read reports), made genuine
  // via `readTraceRecordsFrom`'s own `startedAtLineBoundary` (WS3b Task 2
  // fold-in -- trace-reader.mjs's doc comment): `false` means `minOffset`
  // does not actually land just past a `\n` in the file as it exists RIGHT
  // NOW, i.e. this cursor no longer describes this file's actual history --
  // truncated mid-line and rewritten with different, shorter content, most
  // directly. (An earlier version of this check compared
  // `offsetResult.records.length` against `offsetResult.lineCount` --
  // records can never outnumber lines, so that comparison could never
  // fire; it was a tautology, not a check.) This still isn't a full
  // content re-verification (which would defeat the point of the
  // optimization): a same-length, or otherwise coincidentally boundary-
  // preserving, rewrite that swaps one complete line for different
  // complete-line content still starts at a `\n` and passes this check.
  // That residual gap is the same class of accepted, unmodeled edge case
  // as the module's other restore/rewrite notes above (F8, the
  // shrunk-session guard) -- actions.jsonl is append-only in every real
  // capture path.
  if (!offsetResult.startedAtLineBoundary) return null;

  return {
    readable: true,
    delta: offsetResult.records,
    endByte: offsetResult.endByte,
    usedByteCursor: true,
  };
}

// Resolves the next byte cursor to persist for ONE side (compile or
// provenance) of the two-cursor pair, mirroring the shrink-guarded line
// cursor it corresponds to. Three cases, in order:
// - `nextXLines === observedTotal` (this cursor genuinely caught up this
//   sweep): the freshly-read `endByte` is exactly correct -- use it.
// - `nextXLines === 0` (nothing has ever been consumed on this side, and
//   still hasn't): `0` is always exactly correct for zero records, no
//   fallback needed.
// - Otherwise (frozen mid-way -- a still-live session's compile cursor, or
//   a shrink the Math.max guard just absorbed) AND no prior byte value
//   exists to carry forward (`previousValue` isn't a valid integer, e.g.
//   an old-format entry never touched by this optimization): there is NO
//   trustworthy byte offset for this session's CURRENT `nextXLines` --
//   returns `undefined`, telling the caller to OMIT the key entirely
//   rather than invent `0`. CRITICAL FIX (review round 1, reproduced): the
//   previous version wrote `0` here unconditionally, which silently paired
//   a `bytes: 0` with a nonzero `lines` -- `byteCursorsSane` only checks
//   `bytes <= provenanceBytes`, never bytes-against-lines, so that poisoned
//   pair passed as "sane," the NEXT sweep resumed from byte 0 while still
//   treating the result as a from-`previousLines` delta, and every record
//   already counted got read and counted a second time (a replay's
//   `successRuns` inflating, e.g.). Omitting the key instead makes
//   `byteCursorsSane` correctly refuse it next time (one field absent is
//   enough -- both are required), falling back to a full read that
//   backfills a genuinely correct value once this side actually catches
//   up. Never returns a value smaller than `previousValue` when one
//   exists, by construction (the second branch below just carries it
//   forward unchanged).
function resolveNextByteCursor(nextXLines, observedTotal, endByte, previousValue) {
  if (nextXLines === observedTotal) return endByte;
  if (nextXLines === 0) return 0;
  return Number.isInteger(previousValue) ? previousValue : undefined;
}

// Returns every record from `previousLines` onward -- the SAME slice a
// full `records.slice(previousLines)` would give, obtained via the
// cheapest safe path: the byte-cursor resume above when the cached cursor
// is present and sane, else a full read from byte zero (every sweep's
// original behavior, now the fallback rather than the only path).
// `readable: false` propagates unchanged either way (module doc's
// "unreadable sessions" note: no cursor write, no compile, no provenance).
async function readSessionSinceLines(sessionDir, previous, previousLines) {
  if (byteCursorsSane(previous)) {
    const attempt = await tryByteCursorRead(sessionDir, previous);
    if (attempt) return attempt;
  }

  const full = await readTraceRecordsFrom(sessionDir, 0);
  if (!full.readable) return { readable: false };
  return {
    readable: true,
    delta: full.records.length > previousLines ? full.records.slice(previousLines) : [],
    endByte: full.endByte,
    usedByteCursor: false,
    fullTotal: full.records.length,
  };
}
...
async function loadArtifactRegistry(paths) {
  ...
  // MAT-337: one bucket per journey key (lib/flows/cluster.mjs), holding
  // the NAMES of every artifact that could be the same journey as an
  // incoming flow. Unlike byId/byName this is a multimap -- a bucket can
  // legitimately hold several flows whose steps anchor to DIFFERENT
  // elements despite an identical signature, and only `flowsAreCoJourney`
  // can tell them apart.
  //
  // Names, not locations: a heal landing later in this same sweep rewrites
  // a flow's content and id, and `byName` is this module's already-
  // established way of resolving the FRESHEST copy (see the heal path's own
  // `registry.byName.get(flow.name)` and its comment). A bucket holding
  // location objects would go stale the moment a heal fired, and a merge
  // into a stale copy would silently drop that heal.
  const byJourney = new Map();
  const indexJourney = (flow) => {
    const key = journeyKey(flow);
    const bucket = byJourney.get(key);
    if (!bucket) byJourney.set(key, [flow.name]);
    else if (!bucket.includes(flow.name)) bucket.push(flow.name);
  };
  ...
  return { byId, byName, byJourney, indexJourney };
}

// The freshest artifact in `flow`'s journey bucket that is genuinely the
// same journey, or null. Every bucket name is resolved through `byName`
// (see `indexJourney`'s note on why the bucket holds names) and its journey
// key re-checked against the resolved copy, so a bucket entry that has
// since drifted -- or that names a flow no longer in the registry at all --
// simply does not match rather than corrupting one.
function findCoJourney(flow, registry) {
  const bucket = registry.byJourney.get(journeyKey(flow));
  if (!bucket) return null;
  for (const name of bucket) {
    const location = registry.byName.get(name);
    if (location && flowsAreCoJourney(location.flow, flow)) return location;
  }
  return null;
}
...
// (`flowId`) already exists anywhere in the registry; collapse it into an
// existing artifact if it is the same JOURNEY (MAT-337); rename+re-hash it
// if only its NAME collides; otherwise write it as-is. Returns
// `{ name, tier }` for the sweep report, `{ clusteredInto }` for a
// collapsed twin, or `null` for a dropped exact duplicate.
async function writeCompiledFlow(flow, paths, registry) {
  if (registry.byId.has(flow.id)) return null;

  // MAT-337: BEFORE the name-collision rename below, not after -- the
  // rename is precisely what used to hide these from the exact-content
  // check, since it recomputes the id from the renamed content and so the
  // next identical candidate never matches anything.
  const canonical = findCoJourney(flow, registry);
  if (canonical) {
    const merged = mergeCoJourney(canonical.flow, flow);
    if (merged) {
      const mergedFlow = parseFlow(merged);
      await writeJsonAtomic(canonical.filePath, serializeFlow(mergedFlow));
      const nextLocation = { ...canonical, flow: mergedFlow };
      registry.byId.set(mergedFlow.id, nextLocation);
      registry.byName.set(mergedFlow.name, nextLocation);
    }
    return { clusteredInto: canonical.flow.name };
  }
  ...
  registry.indexJourney(finalFlow);
  return { name: finalFlow.name, tier };
}

// --- replay provenance (ruling c) ---
//
// `isReplayRecord` itself now lives in trace-reader.mjs (WS3b Task 1),
// shared with graph.mjs/inventory.mjs's own replay exclusion -- this used
// to be three independently pinned copies.
...
// past.
//
// WS3b Task 2 fold-in: a mismatch here is NOT simply discarded. It is
// re-proposed ONCE, this time against `current` itself, before giving up.
// WS3a's original "skip" response over-corrected: a stale `flow` reference
// only means the FIRST proposal's `flowId` was computed against outdated
// content, not that the underlying failure evidence is unhealable -- a
// second heal-worthy failure for the SAME flow in one sweep (a different
// step, a different locator for the same step, or literally the same
// replay resolved via a cached pre-heal `{id, name}`) is real signal that
// deserves a real attempt against the flow's CURRENT content, not an
// automatic drop. Re-proposing against `current` trivially satisfies the
// gate whenever it returns non-null (`proposeHeal`'s own `flowId: flow.id`
// field is read straight off the `flow` it was just handed), so this is a
// single, bounded retry against the right data -- never a second
// independent gate, never more than one extra `proposeHeal` call per
// record. The retry itself may still legitimately come back null (e.g. the
// winning locator is already present on `current` -- a duplicate failure
// naming evidence an earlier heal in this same sweep already applied): that
// is the ordinary "no heal, not an error" outcome, same as any other
// unhealable failure, and the record's ordinary provenance write below
...
// WS3b Task 7: `ranker` is `sweep`'s own optional pass-through -- undefined
// when the caller didn't configure a non-lexical encoder, in which case
// `proposeHeal` falls back to its own default (lexical `rankCandidates`)
// exactly as before this task. Every `proposeHeal` call below is now
// `await`ed: `proposeHeal` itself stays synchronous when `ranker` is
// synchronous or absent (heal.mjs's own doc comment), and `await` on a
// plain, already-resolved value is a same-microtask no-op -- so this does
// not change behavior for the lexical-default path, only makes the
// asynchronous encoder-ranker path work.
async function applyReplayRecords(
  replayRecords, registry, updatedByName, healed, healErrors, clock, ranker, paths, runsErrors, sessionDir,
) {
  for (const record of replayRecords) {
    ...
    // A lost browser connection says nothing about whether this flow's
    // locators still work, which is the only thing `failStreak` measures.
    // Counting it walks a healthy flow toward quarantine (match.mjs's
    // threshold) for an infrastructure failure, and because the skill
    // guidance instructs one re-run on SIDECAR_LOST, a single sidecar
    // restart would otherwise spend two of the three strikes. Neither
    // counter moves: it was not a success either.
    //
    // `includes`, not `startsWith`, for the same reason heal.mjs's own
    // parser scans rather than anchors -- the runtime may wrap the macro's
    // throw as `Error: SIDECAR_LOST: {...}` by the time it lands here.
    const sidecarLost = !succeeded && record.error.includes(SIDECAR_LOST_PREFIX);
    let provenance;
    if (succeeded) {
      provenance = {
        ...current.provenance,
        successRuns: current.provenance.successRuns + 1,
        failStreak: 0,
      };
    } else if (sidecarLost) {
      provenance = { ...current.provenance };
    } else {
      provenance = { ...current.provenance, failStreak: current.provenance.failStreak + 1 };
    }

    let nextFlow = { ...current, provenance };
    let healDecision = null;
    // Task 7: the ONLY source of `quirkAttempted` for the runs ledger --
    // flow-runner.js's own key, already present in every FAILURE shape that
    // carries it (no new payload key). `payload` stays `null` on a success
    // (nothing to parse) or when the error string doesn't carry a
    // FLOW_RUNNER_FAILURE payload at all -- either way the ledger entry
    // below reports `quirkAttempted: null`, honestly.
    let payload = null;

    if (!succeeded) {
      payload = parseFailurePayload(record.error);
      if (payload) {
        let proposed = await proposeHeal({ flow, payload, ranker });
        // The gate, immediately before applying: see the BINDING note above.
        if (proposed && proposed.flowId !== current.id) {
          // WS3b Task 2 fold-in: re-propose ONCE against the freshly-read
          // `current` flow before giving up -- see the BINDING note above.
          proposed = await proposeHeal({ flow: current, payload, ranker });
        }
        if (proposed && proposed.flowId === current.id) {
    ...
    // Task 7: whether this record's heal DURABLY landed -- a heal write
    // failure below (contained into `healErrors`) means the proposal was
    // never actually accepted onto disk, so the ledger must report
    // `'failed'`, not `'healed'`, for that record: "healed" means accepted,
    // per the plan's own Scope decisions wording.
    let healAccepted = false;

    try {
      await writeAtomicCore(location.filePath, serializeFlow(nextFlow));
      healAccepted = healDecision !== null;
    } catch (error) {
      if (healDecision) {
        healErrors.push({ name: current.name, error: error?.message ?? String(error) });
        // Task 7: the replay itself still happened and still failed even
        // though the heal that would have followed it never landed -- the
        // ledger records that reality (outcome 'failed') rather than
        // silently dropping this replay from the ledger entirely.
        await appendRunRecord(paths, {
          v: 1,
          ...
          outcome: 'failed',
          quirkAttempted: payload?.quirkAttempted ?? null,
          healedKind: null,
          sessionDir,
        }, runsErrors, sessionDir);
    ...
    // Task 7: one runs-ledger line for this replay, now that its outcome is
    // fully settled (durably written, heal or not). `outcome` precedence
    // (plan Scope decisions): 'healed' when THIS record's own failure was
    // durably healed; 'failed' for any other failure; 'clean' for a
    // success -- see this module's own doc comment above for why
    // 'fallback'/'escalated'/'quirk-recovered' are never reachable here.
    //
    // Review round 1, Important 2 (pinned by test): `flowId`/`flowName`
    // are `current.id`/`current.name` -- the PRE-heal identity this replay
    // was actually run against, not `nextFlow`'s post-heal id (a heal
    // recomputes `id` from the appended locator; `name` never changes
    // across a heal, so `current.name`/`nextFlow.name` are always equal in
    // practice, but `current.*` is used for BOTH fields here, matching the
    // catch branch above, rather than mixing `current.id` with
    // `nextFlow.name` as an earlier draft accidentally did).
    await appendRunRecord(paths, {
      v: 1,
      ts: clock().toISOString(),
      flowName: current.name,
      flowId: current.id,
      origin: current.origin,
      outcome: healAccepted ? 'healed' : (succeeded ? 'clean' : 'failed'),
      quirkAttempted: payload?.quirkAttempted ?? null,
      healedKind: healAccepted ? healDecision.locator.kind : null,
      sessionDir,
    }, runsErrors, sessionDir);
  }
}
...
// origin, reproduced locally (not imported): this one small helper isn't
// worth a shared module of its own the way `isReplayRecord` was (WS3b
// Task 1, now shared via trace-reader.mjs).
...
// sweep({ paths, now?, ranker? }) -> { compiled: [{ name, tier }], clustered,
//   updated:
//   [{ name, successRuns, failStreak }], healed: [{ name, stepIndex, kind }],
...
//   error }] }, runsErrors: [{ sessionDir, error }], warnings: [{ kind,
//   reason }] }
//
// Task 7 (additive): `runsErrors` mirrors `healErrors`'s own containment
// posture for the runs-ledger append (`appendRunRecord`'s doc comment
// above); `warnings` carries `{ kind: 'encoder-degraded', reason }` entries
// whenever the configured `ranker` failed and a heal proposal degraded to
// the lexical default (`warningTrackingRanker`'s doc comment above) -- the
// same shape `flows find`'s own rerank degrade already reports, so a caller
// rendering both (lib/commands/flows.mjs's `compile`, which spreads this
// report verbatim) can treat them identically.
//
// `ranker` (WS3b Task 7, optional, additive) is threaded straight through to
// every `proposeHeal` call this sweep makes for a failed replay -- see
// `applyReplayRecords`'s own doc comment. Omitted, it is `undefined`, which
// `proposeHeal` treats identically to never having been passed at all (its
// own default parameter takes over): a caller that never configured a
// non-lexical encoder sees byte-for-byte the same heal behavior as before
// this task. lib/commands/flows.mjs is the one place that actually resolves
// `config.encoder` and builds a real ranker to pass here -- this module has
// no opinion on where a ranker comes from, only on forwarding it.
//
...
// at risk of tripping.
//
// Trace sessions live directly under `paths.outputDir` (`listTraceSessions`);
...
export async function sweep({ paths, now, ranker } = {}) {
  ...
  const runsErrors = [];
  const warnings = [];
  const trackedRanker = warningTrackingRanker(ranker, warnings);
  ...
  for (const session of sessions) {
    ...
    // WS3b Task 2: `delta` is every record from `previousLines` onward --
    // the SAME slice `records.slice(previousLines)` always gave here,
    // whichever path produced it (see `readSessionSinceLines`'s doc
    // comment). `observedTotal` is what a full read's `records.length`
    // would report right now: reconstructed arithmetically when the
    // byte-cursor path was used (nothing before `previousLines` was even
    // re-read to double check -- that is the whole point), or the full
    // read's own count otherwise.
    const sessionRead = await readSessionSinceLines(session.dir, previous, previousLines);

    if (!sessionRead.readable) {
    ...
    const { delta, endByte, usedByteCursor } = sessionRead;
    const observedTotal = usedByteCursor ? previousLines + delta.length : sessionRead.fullTotal;
    ...
    if (isComplete) {
      ...
      if (compileChunks.length > 0) {
        ...
        for (const compileRecords of compileChunks) {
          ...
          for (const flow of result.flows) {
            ...
            if (!outcome) continue;
            if (outcome.clusteredInto) clustered.push(outcome.clusteredInto);
            else compiled.push(outcome);
          }
        }
        if (skippedForSession.length > 0) skippedBySession[basename] = skippedForSession;

        // Site memory mining (WS3b Task 2: single-pass -- see the module
        // doc comment's "site memory mining" note): every compile chunk
        // CONCATENATED back together, replays already excluded, mined in
        // exactly ONE `mineSiteMemory` call for this session -- never one
        // call per chunk. Segment boundaries above stay compile-only.
        const allCompiledRecords = compileChunks.flat();
        const siteResult = await mineSiteMemory(allCompiledRecords, paths, clock);
      ...
      // `observedTotal` is normally >= `previousLines` by construction
      // (actions.jsonl is append-only), but the ENOENT carve-out (a full
      // read's own honest handling, still reachable via the fallback path
      // above) breaks that: a session whose file vanishes reads as
      ...
      // already `[]` in this case, so nothing new compiled; this just stops
      // the cursor itself from rewinding to match.
      nextLines = Math.max(observedTotal, previousLines);
    }
    // WS3b Task 2: the byte mirror of the shrink-guarded `nextLines` above
    // -- see `resolveNextByteCursor`'s doc comment for the three cases and
    // the critical fix (review round 1) it exists to encode: a byte value
    // with no trustworthy correspondence to `nextLines` is OMITTED, never
    // guessed at with `0`.
    const nextBytes = resolveNextByteCursor(nextLines, observedTotal, endByte, previous?.bytes);
    ...
    // wherever provenance scanning last left off. `delta` already starts at
    // `previousLines`, always `<= previousProvenanceLines` BY THE MODULE'S
    // OWN INVARIANT -- but `Math.max(0, ...)` guards the rare hand-edited/
    // corrupted state where that invariant doesn't hold (a `provenanceLines`
    // smaller than `lines`), which would otherwise slice from a negative
    // index and silently hand a stale TAIL of already-compiled records to
    // provenance scanning instead of an empty "nothing new" slice.
    const provenanceSlice = delta.slice(Math.max(0, previousProvenanceLines - previousLines));
    const replayRecords = provenanceSlice.filter(isReplayRecord);
    if (replayRecords.length > 0) {
      touchedThisSession = true;
      replaysSeen += replayRecords.length;
      await applyReplayRecords(
        replayRecords, registry, updatedByName, healed, healErrors, clock, trackedRanker,
        paths, runsErrors, basename,
      );
    }
    // Same shrink guard as `nextLines` above, applied to the independent
    // provenance cursor, and the same byte-mirroring (and omission rule) as
    // `nextBytes` above.
    const nextProvenanceLines = Math.max(observedTotal, previousProvenanceLines);
    const nextProvenanceBytes = resolveNextByteCursor(
      nextProvenanceLines,
      observedTotal,
      endByte,
      previous?.provenanceBytes,
    );
    ...
    const nextEntry = {
      lines: nextLines,
      provenanceLines: nextProvenanceLines,
    };
    // Omitted (not `0`) when there is no trustworthy value -- see
    // `resolveNextByteCursor`. `byteCursorsSane` requires BOTH present to
    // ever use the fast path, so a lone surviving value here is inert on
    // its own, never misread as a resumable cursor.
    if (nextBytes !== undefined) nextEntry.bytes = nextBytes;
    if (nextProvenanceBytes !== undefined) nextEntry.provenanceBytes = nextProvenanceBytes;
