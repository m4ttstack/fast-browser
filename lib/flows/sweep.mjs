...
// by contrast, is always current (every write -- compile, plain provenance,
// heal -- calls `byName.set(name, ...)` for the SAME name). So: `current`
// below is always resolved fresh via `byName`, and BOTH the ordinary
// provenance update and any heal are built on top of THAT, never the
// (possibly stale) `flow` the record's own id/name happened to resolve to --
// this is what stops a stale-id replay from ever regressing an
// already-healed flow's steps back to their pre-heal shape. The heal gate
// itself then reduces to comparing the proposal's captured pre-heal id
// (from whatever `flow` `resolveReplayTarget` handed back) against
// `current.id` immediately before `applyHeal`: a mismatch means that
// proposal was computed against content `current` has since moved past --
// skip the heal (not an error; the ordinary write still lands, still
...
async function applyReplayRecords(replayRecords, registry, updatedByName, healed, healErrors, clock) {
  for (const record of replayRecords) {
    ...
    const { flow } = location;
    // Always the freshest known copy of THIS flow (by name -- stable across
    // a heal, unlike id) -- see the doc comment above.
    const current = registry.byName.get(flow.name)?.flow ?? flow;
