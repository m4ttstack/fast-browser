...
// WS3b Task 7: runs `ranker` and returns EITHER its result directly (a
// plain `[{index, score}]` array) OR a Promise of one -- `proposeHeal`
// below branches on which it got. Wraps every ranker failure -- a
// synchronous throw, or an async rejection -- in the SAME degrade-to-
// lexical fallback: this is where "ANY encoder failure silently degrades
// that call to lexical" (the WS3b plan's binding degrade rule) actually
// lands for the heal path specifically, so a caller that injects a broken
// or offline encoder-backed ranker still gets a normal lexical heal
// proposal rather than a thrown error or a stuck sweep. `args` is reused
// unchanged for the lexical fallback call -- it's the exact same
// `{ target, candidates }` shape either ranker reads.
function runRanker(ranker, args) {
  let result;
  try {
    result = ranker(args);
  } catch {
    return rankCandidates(args);
  }
  if (result && typeof result.then === 'function') {
    return result.catch(() => rankCandidates(args));
  }
  return result;
}
...
// synthesizable locator (bare text only); or the synthesized locator is
// already among the step's alternates (idempotence -- a sweep may see the
// same failure twice).
//
...
export function proposeHeal({ flow, payload, ranker = rankCandidates }) {
  ...
  // Ranker-supplied thresholds (see the doc comment above): a plain
  // property read, never a function call, so a `ranker` that doesn't carry
  // either property (the lexical default, any test stub) falls straight
  // through to the module constants -- `typeof x === 'number'` also
  // guards against a malformed non-numeric property left on a hand-built
  // stub.
  const minScore = typeof ranker?.minScore === 'number' ? ranker.minScore : HEAL_MIN_SCORE;
  const minMargin = typeof ranker?.minMargin === 'number' ? ranker.minMargin : HEAL_MIN_MARGIN;

  // Turns a completed ranking (`ranked`) into the final decision object --
  // unchanged logic from before Task 7, just factored out so it can run
  // either inline (synchronous ranker) or inside a `.then` (asynchronous
  // ranker) below without duplicating it.
  const finalize = (ranked) => {
