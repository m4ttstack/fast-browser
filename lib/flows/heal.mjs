...
// (kind 'role') when testid is absent; returns null when neither is usable
// -- a candidate whose only evidence is bare `text` is too weak to heal
...
function synthesizeLocator(candidate) {
  ...
  if (role.length > 0 && name.length > 0) {
    return { kind: 'role', selector: `internal:role=${role}[name="${escapeQuotes(name)}"i]` };
  }
  return null;
}
