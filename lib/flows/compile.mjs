...
// '/checkout/{plan}' }` example); `urlPattern` gets the `:argName` path-
// param form the same way. Returns `urlInfo: { origin, patternPath }` for
// the FIRST goto in a segment to seed the flow's top-level `origin`/
...
function buildGotoStep(record, state) {
  ...
  const { templatedQuery, patternQuery } = tokenizeQuery(parsed.search, state);
  const url = `${templatedPath}${templatedQuery}${parsed.hash}`;
  return {
    step: { op: 'goto', url },
    urlInfo: { origin: parsed.origin, patternPath: `${patternPath}${patternQuery}` },
  };
}
...
function buildNonGotoStep(record, state) {
  switch (record.tool) {
    ...
    case 'browser_wait_for': {
      ...
      // no step (segment continues; nothing to build here). A negative or
      // NaN `time` produces a `value` that `parseFlow`'s wait-step
      // validation rejects -- deliberately left to that generic check
      // (fix-round-1 F1) rather than a bespoke guard here, since it's a
      // clean, already-correct rejection path with a clear message.
