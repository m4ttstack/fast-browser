...
function tokenizePath(pathname, state) {
  ...
  for (const segment of segments) {
    ...
    let argName = state.argsByLiteral.get(decoded);
    ...
    if (!argName && isSensitiveUrlValue(decoded, null)) {
      argName = liftWithBaseName(decoded, 'value', state);
    }
    if (argName) {
...
// stays literal when its query key is a non-sensitive one. Left as-is: a
// sensitive KEY name (the branch below) still catches the common real case
// (an actual `token`/`code`/`secret`/... slot), so this is an accepted gap
// rather than a rule this task closes.
//
...
// would). A base64url token's hyphen count is essentially incidental (each
// character has roughly a 1/64 chance of landing on '-'), so two or more
// hyphens is itself a strong signal of human-authored slug data rather
// than token data; the discriminator excludes 2+ hyphens (tightened from
// an initially-proposed ">2" cutoff once the fourth reviewer shape, with
...
function tokenizeQuery(search, state) {
  ...
  for (const pair of pairs) {
    ...
    if (!argName && isSensitiveUrlValue(decoded, key)) {
      argName = liftWithBaseName(decoded, queryKeyBaseName(key), state);
    }
    if (argName) {
...
// all. This applies the identical sensitivity rule to the fragment, in
// whichever of its two common shapes it takes:
//   - a query-string-shaped fragment (one or more `key=value` pairs joined
//     by `&`, the OAuth implicit-grant shape) -- each pair gets exactly
//     `tokenizeQuery`'s own per-pair treatment (sensitive key OR
//     high-entropy value lifts, named from the key);
//   - a single bare value with no `=` anywhere (a path-style deep-link
//     fragment) -- treated as one unkeyed value, lifted only by the
//     high-entropy rule (there's no key to trigger the key-name rule).
// A plain in-page anchor (`#section-2`, `#overview`) needs no special-case
// detection: it's short and wordlike, so neither rule fires and it's
// carried through unchanged, exactly like ordinary path/query data is.
// A value already lifted elsewhere in the flow (a query param, a fill,
// ...) is reused here exactly like every other tokenizer -- this is what
// collapses `?token=X#t=X` onto ONE arg with BOTH occurrences substituted,
// rather than lifting the fragment's copy under a second name.
// `urlPattern` is unaffected either way: it's built from `tokenizePath`
// alone (see fix-round-2 N1 below), and the fragment -- like the query --
// only ever templates the step's own `url`.
function tokenizeFragment(hash, state) {
  ...
  if (body.includes('=')) {
    ...
    for (const pair of pairs) {
      ...
      templated.push(argName ? `${key}={${argName}}` : pair);
    }
