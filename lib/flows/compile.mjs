...
//     (ordinary path data -- "gold", "checkout", a plan slug -- rarely
//     clears all three at once; a base64url JWT or opaque magic-link
//     token routinely does).
...
// credential key name is sufficient on its own regardless of the value's
// shape, otherwise the value must be high-entropy.
function isSensitiveUrlValue(value, queryKey) {
  if (typeof value !== 'string' || value.length === 0) return false;
  if (queryKey && SENSITIVE_QUERY_KEY.test(queryKey)) return true;
