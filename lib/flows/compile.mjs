...
import { FLOW_SCHEMA_VERSION, flowId, parseFlow } from './artifact.mjs';
...
// before being returned, so a compiler bug surfaces as a loud FlowError
// rather than a malformed artifact escaping onto disk.
//
...
// the next source", never as a hard failure.
...
// Absent/short/malformed input degrades to `{ locators: [] }` (rule 3's
// stated fallback shape for a step whose record has no enriched target),
// never throws -- flow-runner is the one that fails loudly at replay.
...
// of the SAME literal -- whether another fill/select value or a goto URL
// segment/query value -- reuses the same arg instead of minting a new one).
// `args` accumulates the flow's `args` map entries directly as they're
// claimed. `uploadCounter` is the separate, always-on sequence for upload
// paths (ruling b), unrelated to name-derived lifting.
...
// (see `liftLiteral`) and short-circuit to the existing arg before ever
// calling this.
...
// Rule 4's lift filter: literals shorter than 2 chars, the string booleans
// `browser_fill_form` uses for checkbox/radio fields ("true"/"false"), and
// anything that isn't a string at all, are not lifted. There is no separate
...
function liftLiteral(literal, target, state) {
  ...
  const baseName = argNameFromTarget(target);
  if (!baseName) return null;
  return claimArgName(camelize(baseName), literal, state);
}
...
// --- rule 4c: goto URL path-segment lifting ---
...
// here, not an independent lift. Query-string values are intentionally left
// untokenized in v1 (see the module-level deviation note near
// `buildGotoStep`) -- the path is the only part of the URL this function
// rewrites.
...
// keeps the literal query string/fragment verbatim (replay needs the real
// destination) while its path segments get `{argName}` templated wherever
// they match an already-lifted literal (matching the fill/select `value`
// placeholder syntax, per the flows-artifact.test.mjs baseFlow fixture's
// own `{ op: 'goto', url: '/checkout/{plan}' }` example). Returns
// `urlInfo: { origin, patternPath }` for the FIRST goto in a segment to
// seed the flow's top-level `origin`/`urlPattern` (rule 9) -- callers only
// use `urlInfo` from the first goto they see.
//
// Deviation from a fully general reading of rule 4c: only path segments are
// tokenized against previously-lifted literals; the query string is passed
// through unmodified in both the step's `url` and `urlPattern`. Rewriting a
// query value in place would require re-serializing via URLSearchParams,
// which percent-encodes the `{argName}`/`:argName` placeholder braces/colon
// and corrupts them -- and neither required golden fixture exercises query
// lifting, so this scope line was drawn deliberately rather than shipping
// an untested query-rewriting path. See task-4-report.md.
function buildGotoStep(record, state) {
  const parsed = safeParseUrl(record?.params?.url);
  if (!parsed) {
    const literal = typeof record?.params?.url === 'string' ? record.params.url : '/';
    return { step: { op: 'goto', url: literal }, urlInfo: null };
  }
  const { templatedPath, patternPath } = tokenizePath(parsed.pathname, state);
  const url = `${templatedPath}${parsed.search}${parsed.hash}`;
  return {
    step: { op: 'goto', url },
...
// a flow with neither available throws rather than fabricating a timestamp,
// since a silently-wrong `compiledAt` would be worse than a loud failure
// here (it feeds directly into `provenance`, which downstream healing/sweep
// logic reasons about).
...
// chronological order: `{ type: 'segment', records }` for a contiguous span
// to attempt compiling, or `{ type: 'error', record }` for a record whose
// `error` field ends the segment at the previous record (rule 1). A segment
// starts at a `browser_navigate` or the first record, and closes right
// before a `browser_navigate` whose URL origin differs from the segment's
// established origin, or at end of trace.
//
...
// as an ordinary segment, evaluated on its own merits (too-short, unsupported,
// or a real flow) with no special treatment beyond having lost its trailing
// record. See task-4-report.md for the reasoning that landed on this
// two-outcomes-per-error model rather than folding the error into the
// preceding segment's own skip reason.
function splitIntoUnits(records) {
  ...
  const flush = () => {
    if (current.length > 0) units.push({ type: 'segment', records: current });
    current = [];
  };
  ...
  for (const record of records) {
    if (record.tool === 'browser_navigate') {
      ...
      } else if (navOrigin && currentOrigin && navOrigin !== currentOrigin) {
        flush();
        currentOrigin = navOrigin;
      }
      current.push(record);
...
function buildNonGotoStep(record, state) {
  switch (record.tool) {
    ...
    case 'browser_select_option': {
      const target = targetFromRecord(record, 0);
      const raw = Array.isArray(record.params?.values) ? record.params.values[0] : undefined;
      const value = fillLikeValue(raw, target, state);
      return applyInteractionExtras({ op: 'select', target, value }, record);
    }
    case 'browser_press_key': {
      const key = typeof record.params?.key === 'string' ? record.params.key : '';
      return applyInteractionExtras({ op: 'press', key }, record);
    }
    case 'browser_hover': {
      const target = targetFromRecord(record, 0);
    ...
    case 'browser_wait_for': {
      ...
      // is MILLISECONDS. text/textGone-only calls are an observation with
      // no step (segment continues; nothing to build here).
      if (typeof record.params?.time === 'number') {
        return { op: 'wait', value: record.params.time * 1000 };
      }
      return null;
    }
    default:
...
// it), named sequentially 'file', 'file2', 'file3', ... across every path
// uploaded in the flow (spanning multiple browser_file_upload records if
// there is more than one). Upload steps carry no `target` (browser_
// file_upload is never one of TRACE.md's seven target-enriched tools, and
// artifact.mjs makes the field optional for exactly this case).
function buildUploadStep(record, state) {
  ...
  const files = paths.map(() => {
    ...
    const name = state.uploadCounter === 1 ? 'file' : `file${state.uploadCounter}`;
...
function buildFlow(records, { meta, fallbackOrigin, traceDir, now, segmentIndex, seqRange }) {
  ...
  return parseFlow(candidate);
}

// --- segment-level compile decision ---

function compileSegment(records, ctx) {
  const seqRange = [records[0].seq, records.at(-1).seq];
  ...
  if (actionCount < 2) return { skip: { reason: 'too-short', seqRange } };

  const flow = buildFlow(records, { ...ctx, seqRange });
  if (!flow) return { skip: { reason: 'too-short', seqRange } };
  return { flow };
}
...
// them). `meta` is `listTraceSessions`'s parsed meta.json (or null).
// `origin` is a fallback web origin used only when a segment can establish
// none of its own (no navigate, and no urlBefore/urlAfter on its first
// record -- an unlikely edge case in practice). `traceDir` and `now` feed
// `provenance` (see `buildFlow`/`resolveCompiledAt`) -- neither is in the
// plan's abbreviated interface sketch, but `provenance.traceDir` and
// `provenance.compiledAt` are both required, non-derivable-from-`records`
// fields, so this module accepts them as additional, optional inputs rather
// than inventing placeholder values silently. See task-4-report.md.
//
...
export function compileSession({ records, meta, origin, traceDir, now } = {}) {
  ...
  for (const unit of units) {
    if (unit.type === 'error') {
      skipped.push({ reason: 'error-truncated', seqRange: [unit.record.seq, unit.record.seq] });
      continue;
    }
    const outcome = compileSegment(unit.records, { meta, fallbackOrigin: origin, traceDir, now, segmentIndex });
