...
// path/query data is. A value already lifted elsewhere in the flow (a
// query param, a fill, ...) is reused here exactly like every other
// tokenizer -- this is what collapses `?token=X#t=X` onto ONE arg with
// BOTH occurrences substituted, rather than lifting the fragment's copy
// under a second name. `urlPattern` is unaffected either way: it's built
// from `tokenizePath` alone (see fix-round-2 N1's own note below), and the
