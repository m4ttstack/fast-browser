...
import { expandLocatorCandidates } from './locators.mjs';
...
function targetFromRecord(record, index) {
  ...
  // MAT-336: the capture layer's alternates are frequently a single
  // over-specified selector, which replays as a hard failure with nothing
  // for heal to rank. See lib/flows/locators.mjs for the ladder and why it
  // is built here rather than at replay.
  target.locators = expandLocatorCandidates(target);
  return target;
}
...
// `REDACTED_ARG_VALUE`. This only WALKS one level (an object's own
// enumerable keys), but the guarantee it gives is not shallow: each
// top-level value is replaced WHOLE, string placeholder in place of
// whatever was there, never merged or recursed into -- so a nested
// object/array value is discarded entirely along with anything it
// contained, not partially redacted down to its own leaves. A secret
// nested two levels deep cannot survive any more than one nested one
// level deep can; there is no depth at which "shallow" leaves a gap.
...
// Scans the whole segment (every record buildFlow received), not only the
// subset that ended up as compiled steps, so this is broader than "compiled
// records only." That divergence is unreachable in practice: a record that
// never becomes a step is one of the non-observed tools TRACE.md documents
// as always reporting mutating: false (browser_fill_form, browser_hover,
// browser_select_option, a plain browser_type, browser_navigate), so
// including it can never flip the result away from 'mutating'. Widening the
// scan only ever errs mutating-ward, the safe direction, so this is left as
// written rather than narrowed to match the compiled-records-only wording.
