...
export function runtimeArgs({ config, paths, lock }) {
  ...
  const args = [
    ...
    '--snapshot-mode=none',
    // MAT-338. The runtime appends a "Ran Playwright code" section to every
    // tool result, and for `browser_run_code_unsafe` that section is the
    // ENTIRE macro file it just executed -- 64k characters echoed back to
    // the agent to report an 84-character flow replay, which is most of the
    // observation saving a one-call replay exists to produce. There is no
    // per-call suppression; this flag is the only switch. Same class of
    // decision as `--snapshot-mode=none` above: the runtime offers to
    // narrate what it did, and an LLM agent reading the result pays for the
    // narration in tokens without ever acting on it.
    '--codegen=none',
    '--timeout-settle=200',
    `--output-dir=${config.outputDir ?? paths.outputDir}`,
  ];
  ...
  // recordings land in ~/.fast-browser/output/videos where the `gif` command reads
