...
import { DOCTOR_CHECK_IDS } from '../doctor/checks.mjs';

const COMMANDS = new Set([
  'setup', 'doctor', 'configure', 'migrate', 'uninstall', 'annotate', 'gif', 'flows', 'sites', 'stats', 'registry',
]);
...
const REGISTRY_SUBCOMMANDS = new Set(['init', 'push', 'pull', 'search', 'status']);

// Per-subcommand flag scoping within `registry` (MAT-160). `requireCommand`
// below only scopes a flag to the COMMAND (`registry` as a whole), which is
// too coarse for a family whose subcommands each consume a different
// subset of the same shared fields (`--origin`, `--intent`, `--yes`,
// `--json`) -- without this, `push --origin` or `status --yes` would parse
// clean and silently do nothing with the value. Enumerated from each
// subcommand's OWN handler in lib/commands/registry.mjs, not guessed: push
// reads `request.yes`; pull and search read `request.origin`; search alone
// reads `request.intent`; status reads none of the four. `init` reads none
// of them either, and is refused `--json` outright here at PARSE time
// rather than only failing later at its own runtime TTY gate
// (lib/commands/registry.mjs's `init`) -- a parse-time refusal is strictly
// narrower than that runtime gate (it can only reject earlier, never admit
// something the gate would have refused) and so cannot regress it.
const REGISTRY_SUBCOMMAND_FLAGS = {
  init: new Set([]),
  push: new Set(['--yes', '--json']),
  pull: new Set(['--origin', '--json']),
  search: new Set(['--intent', '--origin', '--json']),
  status: new Set(['--json']),
};
...
export function parseArgs(argv) {
  ...
  // MAT-160 fix round 1, Important #1: every `--flag` token encountered
  // while `command === 'registry'`, in the order seen, regardless of
  // whether it appeared before or after the subcommand positional.
  // Validated once at the end of the loop below, against `request.sub` at
  // whatever value it ends up holding once the whole invocation has been
  // read.
  const registryFlagTokens = [];
  for (let index = 0; index < arguments_.length; index += 1) {
    ...
    // `registry` takes up to two positionals: the subcommand always comes
    // first (validated against REGISTRY_SUBCOMMANDS), and a second
    // positional -- the registry service URL -- only for `init`. Any other
    // positional (a second one for a non-`init` subcommand, or a third one
    // anywhere) falls through to the bare `throw new UsageError(token)`
    // below, which never echoes the value.
    if (command === 'registry' && !token.startsWith('--')) {
      if (request.sub === null) {
        if (!REGISTRY_SUBCOMMANDS.has(token)) throw new UsageError(token);
        request.sub = token;
        seen.add(token);
        explicitOptions.add(token);
        continue;
      }
      if (request.sub !== 'init') throw new UsageError(token);
      if (request.registryUrl !== null) throw new UsageError(token, 'registry init takes exactly one url argument');
      request.registryUrl = token;
      seen.add(token);
      explicitOptions.add(token);
      continue;
    }
    ...
    seen.add(token);
    explicitOptions.add(token);
    // Registry subcommand strictness (MAT-160): collected here, validated
    // once at the END of the whole parse (below, once `request.sub` is
    // fully resolved) -- NOT here inline. `request.sub` is only set once
    // its own positional token has been consumed, and a flag is free to
    // appear BEFORE that positional (`registry --json status` is exactly
    // as valid as `registry status --json`), so checking against
    // `request.sub` at the moment this flag is seen would reject a
    // perfectly legal prefix-position flag with a nonsensical "not valid
    // for registry <subcommand>" message (fix round 1, Important #1).
    // Every flag token seen while parsing a `registry` invocation is
    // recorded, in order, regardless of position; the whole set is
    // checked once against the subcommand actually resolved.
    if (command === 'registry') registryFlagTokens.push(token);
    switch (token) {
      case '--host': {
      ...
      case '--runtime-lock':
        ...
        break;
      case '--checks': {
        requireCommand(command, ['doctor'], token);
        const ids = valueFor(arguments_, index, token).split(',');
        if (!ids.every((id) => DOCTOR_CHECK_IDS.includes(id))) {
          throw new UsageError(token, `invalid value for ${token}`);
        }
        request.checks = ids;
        index += 1;
      ...
      case '--intent':
        // Shared with `registry` (`search --intent`): same field, same
        // "only the owning command's handler ever reads it" reasoning as
        // `--origin` just below.
        requireCommand(command, ['flows', 'registry'], token);
        request.intent = valueFor(arguments_, index, token);
        index += 1;
        break;
      case '--origin':
        // Shared with `flows` (`find --origin`) and `registry` (`push`/
        // `pull`/`search --origin`): every command's `--origin` lands in
        // the same `request.origin` field, which is safe because each
        // command's own handler is the only thing that ever reads it.
        requireCommand(command, ['flows', 'sites', 'registry'], token);
        request.origin = valueFor(arguments_, index, token);
        index += 1;
        break;
      case '--yes':
        requireCommand(command, ['registry'], token);
        request.yes = true;
        break;
      case '--url':
  ...
  if (command === 'registry' && request.sub === null) {
    throw new UsageError('<subcommand>', 'registry requires a subcommand of init, push, pull, search, or status');
  }
  // Registry subcommand flag strictness (MAT-160), deferred to here so
  // position never matters: `request.sub` is now fully resolved (the
  // check above already refused a `registry` invocation with no
  // subcommand at all), so every flag token collected during the loop --
  // wherever in the invocation it appeared -- is checked against exactly
  // that subcommand's own accepted set.
  if (command === 'registry') {
    const allowedForSub = REGISTRY_SUBCOMMAND_FLAGS[request.sub] ?? new Set();
    for (const token of registryFlagTokens) {
      if (!allowedForSub.has(token)) {
        throw new UsageError(token, `${token} is not valid for registry ${request.sub}`);
      }
    }
  }
  if (command === 'registry' && request.sub === 'init' && request.registryUrl === null) {
    throw new UsageError('<url>', 'registry init requires a url');
  }
  if (
    command === 'registry'
    && request.sub === 'search'
    ...
    throw new UsageError('--intent', 'registry search requires --intent');
  }
