...
export function parseArgs(argv) {
  ...
  for (let index = 0; index < arguments_.length; index += 1) {
    ...
    seen.add(token);
    explicitOptions.add(token);
    // Registry subcommand strictness (MAT-160), checked before the shared
    // switch below: a flag that IS registered for `registry` as a command
    // (so it would otherwise fall through the switch's own coarser
    // `requireCommand` check) but that THIS subcommand's handler never
    // reads is rejected here, naming both the flag and the subcommand.
    if (command === 'registry') {
      const allowedForSub = REGISTRY_SUBCOMMAND_FLAGS[request.sub] ?? new Set();
      if (!allowedForSub.has(token)) {
        throw new UsageError(token, `${token} is not valid for registry ${request.sub ?? '<subcommand>'}`);
      }
    }
    switch (token) {
      case '--host': {
