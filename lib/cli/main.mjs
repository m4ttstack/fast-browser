...
function humanFlowsFind(report) {
  ...
  // lexical order this call. `reason` is mostly encoder.mjs's own fixed,
  // internal diagnostic strings, but NOT uniformly so (fix round 2 review,
  // Folded Minor b): a malformed-JSON failure's message (`voyageEmbed`'s
  // `response.json()` catch) can embed a snippet of the actual server
  // response body via the JSON engine's own SyntaxError text -- server-
  // derived, not this module's own literal, and therefore not trustworthy
  // enough for the `healError.error`-style "internal diagnostic, print
  // verbatim" treatment other warning lines in this file get. Stripped
  // here, unlike those.
  const encoderWarnings = report.warnings.filter((warning) => warning.kind === 'encoder-degraded');
  for (const warning of encoderWarnings) {
    lines.push(`Warning: ranking fell back to lexical order - ${stripControlChars(warning.reason)}`);
  }
  return `${lines.join('\n')}\n`;
}
