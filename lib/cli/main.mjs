...
const HELP = [
  'Usage: fast-browser <setup|doctor|configure|migrate|uninstall|annotate|gif|flows|sites|stats> [options]',
...
function humanStats(report) {
  ...
  const lines = [
    `Replays: ${report.replays} (clean ${outcomes.clean}, fallback ${outcomes.fallback}, `
      + `escalated ${outcomes.escalated}, quirk-recovered ${outcomes['quirk-recovered']}, `
      + `healed ${outcomes.healed}, failed ${outcomes.failed})`,
