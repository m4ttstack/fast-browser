...
const HELP = [
  'Usage: fast-browser <setup|doctor|configure|migrate|uninstall|annotate|gif|flows> [options]',
...
function humanFlowsFind(report) {
  if (report.candidates.length === 0) return 'No matching flows found.\n';
  const lines = report.candidates.map((candidate) => {
    ...
    return `${candidate.name} [${status}] ${candidate.origin} - ${candidate.description}${reasonSuffix}`;
  });
  return `${lines.join('\n')}\n`;
}
...
function humanFlowsCompile(report) {
  ...
  const skipped = Object.values(report.skippedBySession);
  const skippedCount = skipped.reduce((sum, entries) => sum + entries.length, 0);
  if (skippedCount > 0) {
    lines.push(
      `Skipped ${skippedCount} segment(s) across ${skipped.length} session(s); `
      + 'rerun with --json for details.',
    );
  }
  return `${lines.join('\n')}\n`;
