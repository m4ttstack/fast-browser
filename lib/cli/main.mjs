...
// Review fix round 1, Important #4: `result.name`/`result.outcome` here
// are echoed straight from the registry's own POST /v1/push response body
// (registry/lib/http.mjs's `push` handler; ultimately `ingest()`'s return
// value) -- a normal deployment only ever emits kebab-case names and a
// closed outcome vocabulary, but this client treats the registry as
// UNTRUSTED input the same way it treats a pulled envelope's signature, so
// both are stripped before reaching the terminal regardless of what a
...
function humanRegistryPush(report) {
  ...
  for (const result of report.results) {
    const name = stripControlChars(result.name ?? '');
    const outcome = stripControlChars(result.outcome ?? '');
    const reasons = Array.isArray(result.reasons) && result.reasons.length > 0
      ...
      : '';
    lines.push(`${name} - ${outcome}${reasons}`);
  }
...
// `result.outcome`/`result.reason` here are always one of this client's OWN
// fixed literal strings (never echoed from the registry -- see
// lib/commands/registry.mjs's `pull`), so only `result.name` (the pulled
// artifact's own name, server-supplied) needs stripping. `result.note`
// (WS4b Task 7 fix round 1, Important #3) is the same fixed-literal
// treatment: it names the client's OWN `flows approve` requirement, never
// anything the registry chose the wording of.
function humanRegistryPull(report) {
  ...
  for (const result of report.results) {
    ...
    lines.push(`${name} - ${result.outcome}${tier}${reason}${note}`);
  }
...
// Review fix round 1, Important #4: `report.mode` and `result.score` are
// both server-supplied (GET /v1/search's own response body) and were
// interpolated straight into the line with no stripping at all -- `mode`
// is a string field, and `score` is EXPECTED to be a number but this
// client has no way to force the registry to actually send one, so it is
// coerced to a string and stripped exactly like every other server value
// rather than trusted to print safely via template-literal number
// coercion.
function humanRegistrySearch(report) {
  const lines = [`Mode: ${stripControlChars(String(report.mode))}`];
