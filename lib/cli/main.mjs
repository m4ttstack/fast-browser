...
function humanFlowsFind(report) {
  ...
  // lexical order this call. `reason` is one of encoder.mjs's own fixed,
  // internal diagnostic strings (never page-derived text -- see that
  // module's own doc comment), so it prints without a stripControlChars
  // pass, matching healError.error's precedent in humanFlowsCompile below.
  const encoderWarnings = report.warnings.filter((warning) => warning.kind === 'encoder-degraded');
  for (const warning of encoderWarnings) {
    lines.push(`Warning: ranking fell back to lexical order - ${warning.reason}`);
  }
  return `${lines.join('\n')}\n`;
}
