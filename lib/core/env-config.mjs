...
// Every FAST_BROWSER_* variable this process tolerates on the cloud path.
// This has to live next to cloudConfig, not in a shared constants file:
// teaching cloudConfig to read a new variable and registering it here must
// be the same edit, or this allowlist drifts from what the parser actually
// reads and starts rejecting a variable that works.
//
// The bar for membership is NOT "cloudConfig reads it". The check below
// scans whatever env object it was handed, and entry.mjs hands it the whole
// ambient environment, so a variable this parser never reads still has to
// be listed if it can plausibly be exported in the shell that launches the
// runtime. Anything omitted here does not get ignored, it exits 78. Hence
// the second group: FAST_BROWSER_VOYAGE_API_KEY and _REGISTRY_TOKEN are
// read by the flows/registry commands, and RELEASE_DIR and the LIVE_* trio
// are e2e harness opt-ins that tests/e2e/HARNESS.md tells you to export
// (`FAST_BROWSER_RELEASE_DIR=... npm run test:cdp` spreads process.env into
// a child that takes this path). None of them are cloud-contract keys; all
// of them are legitimately present when it runs.
const KNOWN_CLOUD_ENV_VARS = new Set([
  // The cloud contract itself.
  'FAST_BROWSER_ENGINE',
  ...
  // Read by other commands, legitimate alongside a cloud manifest.
  'FAST_BROWSER_VOYAGE_API_KEY',
  'FAST_BROWSER_REGISTRY_TOKEN',
  // Never read here; present in a developer's shell during e2e runs.
  'FAST_BROWSER_RELEASE_DIR',
  'FAST_BROWSER_LIVE_E2E',
  'FAST_BROWSER_LIVE_CDP_URL',
  'FAST_BROWSER_LIVE_EXTENSION_TOKEN',
]);
