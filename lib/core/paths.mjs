...
export function resolvePaths({
  ...
  return {
    ...
    // Everything the runtime writes on its own (trace-*/ session logs,
    // session-*/ transcripts, console-*.log, videos/, and any capture given
    // a relative filename) lands in --output-dir, which launchRuntime pins
    // here. Keeping that below dataDir instead of at it is what keeps the
    // data directory's own files legible; the flow sweep and retention
    // both read this key so the plugin and the runtime agree on the spot.
    outputDir: path.join(dataDir, 'output'),
    screenshotsDir: path.join(dataDir, 'screenshots'),
    // The runtime records --save-video into the `videos` subdirectory of
    // --output-dir; named here so the `gif` command reads where it writes.
