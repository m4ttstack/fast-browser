...
// shelter old ones. The parent sweep leaves the videos/ entry itself alone:
// its root was validated before any removal, so removing it wholesale would
// hand the videos sweep a directory that no longer exists.
async function validateRoots(paths) {
  ...
  const roots = [
    ...
    output && { ...output, parent: data, sessionsOnly: false, childRoots: new Set(['videos']) },
...
export async function pruneSessions({ paths, now, retentionDays, deps = {} }) {
  ...
  for (const root of roots) {
    ...
    for (const name of names) {
      if (root.childRoots?.has(name)) continue;
