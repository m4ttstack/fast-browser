...
// The runtime's --output-dir is the one root that also collects console-*.log
// files, one per tab per navigation; the other two only ever hold session-*
// directories (imported or archived transcripts).
async function validateRoots(paths) {
  ...
  const roots = await Promise.all([
    ...
    validateRealDirectory(expected.output, 'output').then(
      (root) => root && { ...root, consoleLogs: true },
    ),
  ]);
...
async function removableFile(root, name, cutoff) {
  if (!/^console-.*\.log$/.test(name)) return null;
  const logical = path.join(root.logical, name);
  const state = await lstatOrNull(logical);
  ...
  return { logical, physical, state: confirmed, label: 'console log candidate' };
}
  ...
  for (const root of roots) {
    ...
    for (const name of names) {
      ...
      const candidate = directory
        ?? (root.consoleLogs ? await removableFile(root, name, cutoff) : null);
